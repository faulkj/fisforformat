/*!
 * F is for Format, WHAT THE diff, & Friends v2.2
 *
...
window.Date.prototype.f = function (format) {
  ...
  var today = date.getTime() >= midnight.getTime() && date.getTime() < midnight.getTime() + 86400000,
  ...
  var value = {
    ...
    n: M,
    t: new Date(_this.getFullYear(), _this.getMonth(), 0).getDate(),
    L: date.isLeapYear(),
    Y: y,
    y: (y + "").substr(2, 4),
    a: H > 11 ? "pm" : "am",
    ...
    vv: LZ(n, 3),
    e: tz,
    I: _this.getTimezoneOffset() < Math.max(new Date(_this.getFullYear(), 0, 1).getTimezoneOffset(), new Date(_this.getFullYear(), 6, 1).getTimezoneOffset()),
    O: tzO,
...
window.Date.prototype.diff = function (date, settings) {
  if (typeof date == "undefined") date = new Date();else if (typeof date == "string" && !/\d/.test(date) || _typeof(date) == "object" && !date.getTime) {
    settings = date;
    date = new Date();
  } else if (typeof date == "string" || typeof date == "number") {
    if (new Date(date) != "Invalid Date" && new Date(date) != "NaN") date = new Date(date);else {
  ...
  var min = date <= _this && date || date > _this && _this,
      max = date > _this && date || date <= _this && _this,
  ...
  for (var i = 0; i < settings.units.length; i++) {
    ...
    if (res || settings.zeros) {
      if (settings.zeros && settings.units.charAt(i) == "N") {
        if (res < 10) res = "00" + res;else if (res < 100) res = "0" + res;
      }
...
window.Date.prototype.happened = function () {
  ...
  var now = new Date(),
      ts = _this.f("F j, Y g:i a"),
      nowSec = now.getTime() / 1000,
      tsSec = _this.getTime() / 1000;

  if (nowSec - tsSec < 20) ts = "Now";else if (nowSec - tsSec < 60) ts = _this.diff(now, "s");else if ((nowSec - tsSec) / 60 < 60) ts = _this.diff(now, "m");else if ((nowSec - tsSec) / 60 / 60 < 12) ts = _this.diff(now, "H");else if (now.f("Ymd") == _this.f("Ymd")) ts = "Today " + _this.f("g:i a");else if (tsSec > new Date(new Date(now.f("m/d/Y 00:00:00")).setDate(now.getDate() - 1)).getTime() / 1000) ts = "Yesterday " + _this.f("g:i a");else if (tsSec > new Date(new Date(now.f("m/d/Y 00:00:00")).setDate(now.getDate() - now.getDay())).getTime() / 1000) ts = _this.f("l g:i a");else if (tsSec > new Date(new Date().getFullYear(), 0, 1).getTime() / 1000) ts = _this.f("F j g:i a");
  return ts;
};
/********* Date.getDaysInMonth() *************
...
window.Date.prototype.getDaysInMonth = function () {
  return new Date(_this.getFullYear(), _this.getMonth(), 0).getDate();
};
/************* Date.isLeapYear() ***************
...
window.Date.prototype.isLeapYear = function () {
  return new Date(_this.getFullYear(), 1, 29).getDate() == 29;
};
...
window.Date.prototype.isDayLightSavings = function () {
  return _this.getTimezoneOffset() < Math.max(new Date(_this.getFullYear(), 0, 1).getTimezoneOffset(), new Date(_this.getFullYear(), 6, 1).getTimezoneOffset());
};
...
window.Date.prototype.getDaylightSavingsDays = function () {
  var result = [],
      day1 = new Date("03/07/" + _this.getFullYear()),
      day2 = new Date("03/06/" + _this.getFullYear());
  ...
  day1 = new Date("10/31/" + _this.getFullYear());
  day2 = new Date("10/30/" + _this.getFullYear());
...
window.Date.prototype.isDaylightSavingsDay = function () {
  var c = new Date(_this.getTime());
  c.setDate(c.getDate() + 1);
  return (c.getTime() - _this.getTime()) / 1000 / 60 / 60 != 24;
};